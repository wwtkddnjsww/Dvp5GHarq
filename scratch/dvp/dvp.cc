...
main(int argc, char* argv[])
{
    ...
    uint32_t udpPacketSizeULL = 68; //orig 100
    uint32_t lambdaULL = 3; // The number of packets in one second 8ms: 125, 2ms=500
    uint16_t srsPeriodicity = 80; // in ms, for SRS transmission periodicity. 10ms is the default value in the LteEnbRrc class, and it is used for both LTE and NR simulations. For more details, please check the LteEnbRrc class documentation.
    // Simulation parameters. Please don't use double to indicate seconds; use
    // ns-3 Time values which use integers to avoid portability issues.
    Time simTime = MilliSeconds(250);//1600400= 1million simtime for 8ms case = 800 second. 2ms case = 200 second (160400)
    Time udpAppStartTime = MilliSeconds(200);
    uint16_t channelCoherence = 1;
    std::string pattern = "DL|UL|DL|UL|DL|UL|DL|UL|DL|UL|"; 
    ...
    double bandwidthBand1 = 100e6; // 100MHz for single and 20MHz for 10 UEs
    double totalTxPower = -2.51604; //orig 4, for single transmission -10dbm
    ...
    cmd.AddValue("totalTxPower",
                 ...
                 totalTxPower);
    cmd.AddValue("srsPeriodicity",
                 "SRS Periodicity"
                 "in millisecond, range 80-320",
                 srsPeriodicity);
    ...
    cmd.Parse(argc, argv);
    // std::cout << "MAX Packet DUP : " << int(maxPacketDup) << std::endl;
    /*
    ...
    Config::SetDefault("ns3::NrMacSchedulerSrsDefault::StartingPeriodicity", UintegerValue(srsPeriodicity));
    ...
    else
    {
        ...
        for (uint32_t kk = 0; kk <= ueNum/10; kk++){
            ...
            {
                uePositionAlloc->Add(Vector(k*1.0, kk,ueHeight));
            }
        }
    ...
    nrHelper->SetSchedulerAttribute("EnableUrllcHarqReTx", BooleanValue(false)); 
    ...
    nrHelper->GetUePhy(ueLowLatNetDev.Get(0), 0)
    ->SetAttribute("TxPower", DoubleValue(10 * log10(x)));
    ...
    // serverApps.Add(dlPacketSinkLowLat.Install(ueLowLatContainer)); changed
    ...
    dlClientLowLat.SetAttribute("Interval", TimeValue(MilliSeconds(lambdaULL)));
