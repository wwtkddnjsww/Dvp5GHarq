...
class LtePdcp : public Object // SimpleRefCount<LtePdcp>
{
  ...
  public:
    ...
    void SetNumberOfDuplications(uint8_t dupNum);

    void SetDuplicationDelay(uint16_t dupDelay);

    uint8_t GetNumberOfDuplications();

    uint8_t GetDuplicationDelay();

    uint8_t GetMaxNumberOfDuplications();

    bool IsFixedDuplication();

    void SetUrllcUes(uint8_t v);
    uint8_t GetUrllcUes();
    /**
  ...
  protected:
    ...
    uint8_t m_lcid;  ///< LCID
    // uint16_t pdcpSequencNumbers[5000]; //changed by Sangwon
    uint16_t accumulated_dup_vals = 0; //changed by Sangwon
    uint16_t accumulated_seq_vals = 0; //changed by Sangwon
    /**
  ...
  private:
     ...
     */
    static const uint16_t m_maxPdcpSn = MAX_PDCP_SN;
    
    //changed by Sangwon
    uint8_t m_maxDup = 0;
    uint8_t m_numberOfDuplications = 0;
    uint16_t m_dupDelay = 2;
    BooleanValue m_isFixedDuplication = true;
    uint8_t m_urllcUes = 5;
    bool m_maxSeqNotifier= false; // This variable is used after the sequence number initialized but duplicated packets still arrives;
};
