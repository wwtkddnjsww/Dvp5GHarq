...
UeManager::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::UeManager")
            ...
            .AddAttribute("CaDuplication",
                          "Carrier aggregation duplication",
                          BooleanValue(false), // unused, read-only attribute
                          MakeBooleanAccessor(&UeManager::m_caDuplication),
                          MakeBooleanChecker())
...
UeManager::SendPacket(uint8_t bid, Ptr<Packet> p)
{
    ...
    auto it = m_drbMap.find(drbid);
    // std::cout<<std::endl<<int(drbid)<<std::endl<<m_drbMap.size()<<std::endl<<std::endl;
    //this part need to change to CCMAP
    if (it != m_drbMap.end())
    {
        if (m_caDuplication){    
            for (uint8_t i=2; i <= m_drbMap.size(); i ++){
                if (drbid != i){
                    Ptr<Packet> tempPacket = Copy(p);
                    LtePdcpSapProvider::TransmitPdcpSduParameters tempParams;
                    tempParams.pdcpSdu = tempPacket;
                    tempParams.rnti = m_rnti;
                    tempParams.lcid = Bid2Lcid(i);
                
                    Ptr<LteDataRadioBearerInfo> tempBearerInfo = GetDataRadioBearerInfo(i);
                    if (tempBearerInfo){
                        NS_LOG_INFO("Send duplicated packet to different PDCP layer that combined with DRB");
                        LtePdcpSapProvider* pdcpSapProvider = tempBearerInfo->m_pdcp->GetLtePdcpSapProvider();
                        pdcpSapProvider->TransmitPdcpSdu(tempParams);
                    }
                }
            }
        }
