    ...
    static TypeId tid =
        TypeId("ns3::LteRlcUm")
            ...
            .AddAttribute("DiscardTimerMs",
                          ...
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("IndependentChannelInstanceForSinglePaket",
                          "This attribute is used for the packet scheduling that RLC UM will"
                          "report buffer status as one in order to send one packet per one slot"
                          "in slingle UE.",
                          BooleanValue(false),
                          MakeBooleanAccessor(&LteRlcUm::m_independent_queue),
                          MakeBooleanChecker())
            .AddAttribute("SchedulingDiscardTimerMs",
                          ...
                          UintegerValue(0),
                          MakeUintegerAccessor(&LteRlcUm::m_schedulingDiscardTimerMs),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("DiscardingRepetitionPacket",
                          "Discard Repetition Packet when RLC buffer still keeps original packet.",
                          BooleanValue(false),
                          MakeBooleanAccessor(&LteRlcUm::m_DiscardingRepetitionPacketInTheSameQueue),
                          MakeBooleanChecker());
    return tid;
}

void
LteRlcUm::PacketDeadlineTimer() {
    ...
    if (!m_txBuffer.empty())
    {
        ...
        // Remove repeititon packets if original packet still in the RLC buffer 
...
LteRlcUm::DoTransmitPdcpPdu(Ptr<Packet> p)
{
    ...
    if (m_txBufferSize + p->GetSize() <= m_maxTxBufferSize)
    {
        ...
        // std::cout <<std::endl<<"pdcp tag time:" << pdcptag.GetSenderTimestamp() << "cur time: "<< Simulator::Now() << std::endl;;
        ...
        // Sender timestamp has been changed due to the pdcp duplication.
        // re ordering the packets according to PDCP layer
        // if (m_txBuffer.empty()){
        //     m_txBuffer.emplace_back(p, pdcptag.GetSenderTimestamp());
        // }
        // else{
        ...
        // }
        // \todo is change this syntax working! Currently, it throws error
        // else{
        //     LtePdcpHeader it_pdcpHeader;
        //     LtePdcpHeader cur_pdcpHeader; 
        //     p->PeekHeader(cur_pdcpHeader);
        //     int insert_val = 0;
        //     for (auto it= m_txBuffer.begin(); it != m_txBuffer.end(); it ++){
        //         it->m_pdu->PeekHeader(it_pdcpHeader);
        //         if (it_pdcpHeader.GetSequenceNumber() <= cur_pdcpHeader.GetSequenceNumber()){
        //             ++insert_val;
        //         }
        //     }
        //     m_txBuffer.insert(m_txBuffer.begin()+insert_val, TxPdu(p, pdcptag.GetSenderTimestamp()));
        ...
        Simulator::Schedule(MilliSeconds(1),
            &LteRlcUm::PacketDeadlineTimer, this);
        // }
    }
    else
    {
...
LteRlcUm::DoNotifyTxOpportunity(LteMacSapUser::TxOpportunityParameters txOpParams)
{
    ...
    {
        m_rbsTimer.Cancel();
        m_rbsTimer = Simulator::Schedule(MilliSeconds(1), &LteRlcUm::ExpireRbsTimer, this);
    }
}
...
{
    // PacketDeadlineTimer(); // this will be added the packets according to the deadline
    Time holDelay(0);
    ...
    if (m_independent_queue == true){
        // Here, we make the queue size as the single packet (in order to make independent channel)
        // Therefore, the queue size is set to the first packet size (m_txBuffer[0].m_pdu->GetSize())
        // and expected header size (2) .
        uint8_t qlength = m_txBuffer.size();
        queueSize = m_txBuffer[qlength - 1].m_pdu->GetSize() + 2;
    }
