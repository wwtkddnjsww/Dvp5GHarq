...
UdpServer::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::UdpServer")
            ...
            .AddAttribute("ResultFilename",
                          "Name of file to save trace.",
                          StringValue(""),
                          MakeStringAccessor(&UdpServer::SetPath),
                          MakeStringChecker())
...
void
UdpServer::SetPath(std::string path){
    m_filepath = path;
}
    ...
    while ((packet = socket->RecvFrom(from)))
    {
        ...
        if (packet->GetSize() > 0)
        {
            ...
            if (InetSocketAddress::IsMatchingType(from))
            {
                ...
                outfile<<"TraceDelay: RX " << receivedSize << " bytes from "
                                              << InetSocketAddress::ConvertFrom(from).GetIpv4()
                                              << " Sequence Number: " << currentSequenceNumber
                                              << " Uid: " << packet->GetUid() << " TXtime: "
                                              << seqTs.GetTs() << " RXtime: " << Simulator::Now()
