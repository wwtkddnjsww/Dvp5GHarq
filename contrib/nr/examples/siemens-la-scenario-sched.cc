...
main(int argc, char* argv[])
{
    ...
    uint16_t gNbNum = 1;
    uint16_t ueNum = 20; //10 for urllc
    uint16_t urllcUes = 0;
    uint16_t positionResetTime = 2000; // Reset time for 8ms case = (8000ms), 2ms case = 2000ms.
    ...
    bool schedRrcConnectionEnabled=true;
    // Traffic parameters (that we will use inside this script):
    uint32_t udpPacketSizeULL = 100; //orig 100
    uint32_t lambdaULL = 125; // The number of packets in one second 8ms: 125, 2ms=500
    uint32_t udpPacketSizeBe = 27500;
    uint32_t lambdaBe = 50;
    // Simulation parameters. Please don't use double to indicate seconds; use
    // ns-3 Time values which use integers to avoid portability issues.
    Time simTime = MilliSeconds(40400);//1600400= 1million simtime for 8ms case = 800 second. 2ms case = 200 second (160400)
    ...
    cmd.AddValue("ReorderingTimer",
                ...
                ReorderingTimerVal);
    cmd.AddValue("StochasticInterval",
                 "Function for UDP Stochastic",
                 stochasticInterval);
    ...
    if (schedulerType == "ns3::NrMacSchedulerOfdmaEdf"){
        // Config::SetDefault("ns3::LteRlcTm::DiscardingRepetitionPacket", BooleanValue(true));
        Config::SetDefault("ns3::LteRlcTm::SchedulingDiscardTimerMs", UintegerValue(schedulingDiscardTimerMs));
    }
    urllcUes = 20;
    Config::SetDefault("ns3::LtePdcp::UrllcUes", UintegerValue(urllcUes));
    useFixedMcs = true;
    ...
    int64_t randomStream = 30;
    double ueHeight = 1.5;
    GridScenarioHelper gridScenario;
    gridScenario.SetStartingPosition(Vector(50,50,0));
     ...
     * while in ueMcVideo we will put the UEs that will receive the voice traffic.
     */
    ...
    else
    {
        ...
        for (uint32_t kk = 0; kk <= ueNum/10; kk++){
            ...
            {
                if (uePositionAlloc->GetSize() >= ueNum)
                {
                    break;
                }
                uePositionAlloc->Add(Vector(20.0-k*1.0, 20.0-kk,ueHeight));
            }
        }
    ...
    NodeContainer ueMcVideoContainer;
    
    for (uint32_t j = 0; j < urllcUes ; ++j)
    {
        Ptr<Node> ue = gridScenario.GetUserTerminals().Get(j);
        ueLowLatContainer.Add(ue);
    }
    std::cout<<"N terminals" << gridScenario.GetUserTerminals().GetN();
    for (uint32_t j = urllcUes; j < gridScenario.GetUserTerminals().GetN() ; ++j)
    {
        Ptr<Node> ue = gridScenario.GetUserTerminals().Get(j);
        ueMcVideoContainer.Add(ue);
    }
    ...
    // }
    nrHelper->SetSchedRrcConnectionEnabled(schedRrcConnectionEnabled);
    ...
    nrHelper->SetSchedulerAttribute("EnableUrllcHarqReTx", BooleanValue(isHarqEnabled)); 
    ...
    if (useFixedMcs == true)
    {
        ...
        nrHelper->SetSchedulerAttribute("UrllcUes", UintegerValue(urllcUes));
    }
    ...
    nrHelper->SetGnbBwpManagerAlgorithmAttribute("GBR_MC_VIDEO", UintegerValue(bwpIdForMcVideo));

    // Ue routing between Bearer and bandwidth part
    nrHelper->SetUeBwpManagerAlgorithmAttribute("NGBR_LOW_LAT_EMBB", UintegerValue(bwpIdForLowLat));
    nrHelper->SetUeBwpManagerAlgorithmAttribute("GBR_MC_VIDEO", UintegerValue(bwpIdForMcVideo));
    ...
    if (caDuplication == true){
        ...
        // Ue routing between Bearer and bandwidth part
        nrHelper->SetUeBwpManagerAlgorithmAttribute("NGBR_LOW_LAT_EMBB", UintegerValue(bwpIdForLowLatPacketDup));
    }
    /*
    ...
    NetDeviceContainer ueMcVideoNetDev = nrHelper->InstallUeDevice(ueMcVideoContainer, allBwps);

    randomStream += nrHelper->AssignStreams(enbNetDev, randomStream);
    randomStream += nrHelper->AssignStreams(ueLowLatNetDev, randomStream);
    randomStream += nrHelper->AssignStreams(ueMcVideoNetDev, randomStream); // This is mission critical video
    ...
    for (auto it = ueMcVideoNetDev.Begin(); it != ueMcVideoNetDev.End(); ++it)
    {
        DynamicCast<NrUeNetDevice>(*it)->UpdateConfig();
    }
    ...
    Ipv4InterfaceContainer ueMcVideoIpIface =
        epcHelper->AssignUeIpv4Address(NetDeviceContainer(ueMcVideoNetDev));
    ...
    nrHelper->AttachToClosestEnb(ueMcVideoNetDev, enbNetDev);
    /*
     * Traffic part. Install traffic: low-latency
     * identified by a particular source port.
     */
    uint16_t dlPortLowLat = 1234;
    uint16_t dlPortMcVideo = 1235;

    ApplicationContainer serverApps;

    // The sink will always listen to the specified ports
    UdpServerHelper dlPacketSinkLowLat(dlPortLowLat);
    UdpServerHelper dlPacketSinkMcVideo(dlPortMcVideo);
    ...
    serverApps.Add(dlPacketSinkMcVideo.Install(ueMcVideoContainer));
    /*
    ...
    dlClientLowLat.SetAttribute("StochasticInterval",BooleanValue(stochasticInterval));

    // The bearer that will carry low latency traffic
    EpsBearer lowLatBearer(EpsBearer::NGBR_LOW_LAT_EMBB);
    EpsBearer lowLatBearerPacketDup(EpsBearer::NGBR_LOW_LAT_EMBB);
    ...
    // McVideo configuration and object creation:
    UdpClientHelper dlClientMcVideo;
    dlClientMcVideo.SetAttribute("RemotePort", UintegerValue(dlPortMcVideo));
    dlClientMcVideo.SetAttribute("MaxPackets", UintegerValue(0xFFFFFFFF));
    dlClientMcVideo.SetAttribute("PacketSize", UintegerValue(udpPacketSizeBe));
    dlClientMcVideo.SetAttribute("Interval", TimeValue(Seconds(1.0 / lambdaBe)));

    // The bearer that will carry voice traffic
    EpsBearer mcVideoBearer(EpsBearer::GBR_MC_VIDEO);

    // The filter for the voice traffic
    Ptr<EpcTft> mcVideoTft = Create<EpcTft>();
    EpcTft::PacketFilter dlpfMcVideo;
    dlpfMcVideo.localPortStart = dlPortMcVideo;
    dlpfMcVideo.localPortEnd = dlPortMcVideo;
    mcVideoTft->Add(dlpfMcVideo);
    ...
    for (uint32_t i = 0; i < ueMcVideoContainer.GetN(); ++i)
    {
        Ptr<Node> ue = ueMcVideoContainer.Get(i);
        Ptr<NetDevice> ueDevice = ueMcVideoNetDev.Get(i);
        Address ueAddress = ueMcVideoIpIface.GetAddress(i);

        // The client, who is transmitting, is installed in the remote host,
        // with destination address set to the address of the UE
        dlClientMcVideo.SetAttribute("RemoteAddress", AddressValue(ueAddress));
        clientApps.Add(dlClientMcVideo.Install(remoteHost));

        // Activate a dedicated bearer for the traffic type
        nrHelper->ActivateDedicatedEpsBearer(ueDevice, mcVideoBearer, mcVideoTft);
    }
    ...
    Time additional_sim_time = MilliSeconds(12); // additional time for the rest of packet
