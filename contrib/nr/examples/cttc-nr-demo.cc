...
{
    //LogComponentEnable("NrNetDevice", LOG_LEVEL_INFO);
    ...
    uint32_t lambdaULL = 1000; // The number of packets in one second
    uint32_t lambdaBe = 400;

    // Simulation parameters. Please don't use double to indicate seconds; use
    // ns-3 Time values which use integers to avoid portability issues.
    Time simTime = MilliSeconds(900);
    ...
    Time scheduling_delay = MilliSeconds(100); // make some delay
    Simulator::Stop(simTime+scheduling_delay);
    Simulator::Run();
