...
main(int argc, char* argv[])
{
    ...
    Time simTime = MilliSeconds(160400);// simtime for 8ms case = 800 second. 2ms case = 200 second
    ...
    double bandwidthBand2 = 80e6; // 100MHz for single and 20MHz for 10 UEs
    ...
    cmd.AddValue("StochasticInterval",
                 ...
                 stochasticInterval);
    cmd.AddValue("NoiseAnalysis",
                "Print the noise for analysis",
                noiseAnalysis);
    ...
    // Config::SetDefault("ns3::NrInterference::NoiseAnalysis", BooleanValue(noiseAnalysis));
    /*
     * In general, attributes for the NR module are typically configured in NrHelper.  However, some
     * attributes need to be configured globally through the Config::SetDefault() method. Below is
     * an example: if you want to make the RLC buffer very large, you can pass a very large integer
    ...
    else
    {
        ...
        for (uint32_t kk = 0; kk <= ueNum/10; kk++){
            ...
            {
                uePositionAlloc->Add(Vector(20.0-k*1.0, 20.0-kk*0.5,ueHeight));
            }
        }
