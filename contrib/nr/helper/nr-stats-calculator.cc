...
void
NrStatsCalculator::SetResultPath(std::string path){
    m_path = path;
}
...
NrStatsCalculator::SetUlOutputFilename(std::string outputFilename)
{
    if (m_path.length() == 0){
        ...
        m_ulOutputFilename = m_path+outputFilename;
    }
}
...
NrStatsCalculator::SetDlOutputFilename(std::string outputFilename)
{
    if (m_path.length() == 0){
        ...
        m_dlOutputFilename = m_path+outputFilename;
    }
}
...
NrStatsCalculator::FindImsiFromGnbRlcPath(std::string path)
{
    ...
    // We retrieve the  associated to the C-RNTI and perform the IMSI lookup
