...
NrHelper::GetTypeId()
{
    static TypeId tid = TypeId("ns3::NrHelper")
                            ...
                            .AddAttribute("HarqEnabled",
                                          ...
                                          MakeBooleanChecker())
                            .AddAttribute("UrllcHarqEnabled",
                                          "Enable Hybrid ARQ for URLLC packets",
                                          BooleanValue(true),
                                          MakeBooleanAccessor(&NrHelper::m_urllcHarqEnabled),
                                          MakeBooleanChecker())
                            .AddAttribute("SchedRrcConnectionEnabled",
                                          "Enable Hybrid ARQ",
                                          BooleanValue(false),
                                          MakeBooleanAccessor(&NrHelper::m_SchedRrcConnectionEnabled),
                                          MakeBooleanChecker());
    return tid;
}

void
NrHelper::SetSchedRrcConnectionEnabled(bool v){
    m_SchedRrcConnectionEnabled = v;
}
...
void
NrHelper::SetUrllcUes(uint8_t urllcUes)
{
    m_urllcUes = urllcUes;
}

uint8_t
NrHelper::GetUrllcUes()
{
    return m_urllcUes;
}
void
...
Ptr<NrMacScheduler>
NrHelper::CreateGnbSchedRrcConnected(Ptr<LteEnbRrc> rrc)
{
    NS_LOG_FUNCTION(this);
    ...
    uint8_t numberOfStreams)
{
    ...
    //Ptr<LteEnbRrc> rrc = CreateOjbect<LteEnbRrc>();
    ...
    for (uint32_t bwpId = 0; bwpId < allBwps.size(); ++bwpId)
    {
        ...
        if (m_SchedRrcConnectionEnabled == true){
            auto sched = CreateGnbSchedRrcConnected(rrc);
    ...
    if (m_epcHelper != nullptr)
    {
        ...
        else if (epsBearerToRlcMapping.Get() == LteEnbRrc::RLC_TM_ALWAYS)
        {
            rrc->SetAttribute("EpsBearerToRlcMapping", EnumValue(LteEnbRrc::RLC_TM_ALWAYS));
        }
    }
