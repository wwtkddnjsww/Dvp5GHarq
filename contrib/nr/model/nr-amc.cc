        ...
        TypeId("ns3::NrAmc")
            ...
            .AddAttribute("AmcModel",
                          ...
                          MakeEnumChecker(NrAmc::ErrorModel,
                                          ...
                                          NrAmc::ShannonModel,
                                          "ShannonModel"))
            .AddAttribute("TargetErrorRate",
                          "Target Error rate for Link Adaptation Algorithms",
                          DoubleValue(0.1),
                          MakeDoubleAccessor(&NrAmc::SetTargetErrorRate, &NrAmc::GetTargetErrorRate),
                          MakeDoubleChecker<double>(0.0, 1.0))  // Changed by Sangwon
...
NrAmc::CalculateTbSize(uint8_t mcs, uint32_t nprb) const
{
    ...
    // std::cout << " mcs:" << (unsigned)mcs << " TB size:" << tbSize << std::endl;
    return tbSize;
}
...
NrAmc::CreateCqiFeedbackWbTdma(const SpectrumValue& sinr, uint8_t& mcs) const
{
    ...
    else if (m_amcModel == ErrorModel)
    {
        ...
        while (mcs <= m_errorModel->GetMaxMcs())
        {
            ...
            if (output->m_tbler > m_targetErrorRate) //Changed by Sangwon
            {
                break;
            }
            mcs++;
        ...
        if ((output->m_tbler > m_targetErrorRate) && (mcs == 0)) //Changed by Sangwon
        {
            cqi = 0;
        }
...
void
NrAmc::SetTargetErrorRate(const double targetErrorRate)
{
    NS_LOG_FUNCTION(this);
    m_targetErrorRate = targetErrorRate;    
}

double
NrAmc::GetTargetErrorRate() const
{
    NS_LOG_FUNCTION(this);
    return m_targetErrorRate;
}
