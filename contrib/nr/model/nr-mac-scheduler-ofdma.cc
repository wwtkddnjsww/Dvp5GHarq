...
NrMacSchedulerOfdma::AssignDLRBG(uint32_t symAvail, const ActiveUeMap& activeDl) const
{
    ...
    for (const auto& el : activeDl)
    {
        ...
        {
            GetFirst GetUe;
            // This is actual function calling of UE vectors that scheduling comparing is happen in here.
            ...
            // std::cout<< "resources" << resources << ", RNTI: " << GetUe(*schedInfoIt)->m_rnti << std::endl;
    ...
    for (const auto& el : activeUl)
    {
        ...
        while (resources > 0)
        {
            ...
            NS_LOG_DEBUG("Assigned " << rbgAssignable << " UL RBG, spanned over " << beamSym
                                     << " SYM, to UE " << GetUe(*schedInfoIt)->m_rnti);
            // std::cout << "Assigned " << rbgAssignable << " UL RBG, spanned over " << beamSym
            //                          << " SYM, to UE " << GetUe(*schedInfoIt)->m_rnti << std::endl;
