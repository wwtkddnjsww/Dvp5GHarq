...
{ 
  public:
    /**
     * \brief NrMacSchedulerUeInfoEdf constructor
     ...
     *
     * It calls also NrMacSchedulerUeInfoEdf::ResetDlSchedInfo.
     */
     ...
     * and zeroes the average throughput as well as the current throughput.
     *
     * It also calls NrMacSchedulerUeInfoEdf::ResetUlSchedInfo.
     */
    ...
    static double CalculateDlWeight(const NrMacSchedulerNs3::UePtrAndBufferReq& ue)
    {
        ...
        for (const auto& ueLcg : ue.first->m_dlLCG)
        {
            ...
            for (const auto lcId : ueActiveLCs)
            {
                ...
                uint16_t delayBudget = LCPtr->m_delayBudget.GetMilliSeconds();
                uint16_t priority = LCPtr->m_priority; // if URLLC, it is 68
                // std::cout<<"priority" << priority<< std::endl;
                
                double targetPER = LCPtr->m_PER;
                // std::cout<<"targetPer" << targetPER << std::endl;
                double delayBudgetFactor = delayBudget - LCPtr->m_rlcTransmissionQueueHolDelay;
                if (delayBudgetFactor < 0){
                    delayBudgetFactor = 0;
                }
                else if (delayBudgetFactor > 100)
                {
                    delayBudgetFactor = 100;
                    /* code */
                }
                
                if (priority == 68){ // case of URLLC
                    weight += (100 - delayBudgetFactor)/targetPER;
                }
                else{ // non URLLC bit cases.
                    if (weight == 0){
                        weight += (100 - delayBudgetFactor)/targetPER;
                    }
                }
                // std::cout << "UE id: " << int(lcId) << ", delay factor: " << delayBudgetFactor << std::endl;
