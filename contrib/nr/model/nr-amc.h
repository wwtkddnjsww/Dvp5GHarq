...
class NrAmc : public Object
{
  public:
    ...
    /**
     * \brief Set the target error rate
     * \param targetErrorRate the double value of a target error rate in range 0 to 1
     */
    void SetTargetErrorRate(const double targetErrorRate);
    /**
     * \brief Get the target error rate
     * \return error rate
     */
    double GetTargetErrorRate() const;
  ...
  private:
    ...
    double m_targetErrorRate;                      //!< Target Error rate (should be 0 to 1); Change by Sangwon
