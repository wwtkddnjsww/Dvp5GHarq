...
class NrMacSchedulerNs3 : public NrMacScheduler
{
  public:
    ...
    // these functions are used for duplication
    ...
    void EnableUrllcHarqReTx(bool enableFlag);
    bool IsUrllcHarqReTxEnable() const;
  ...
  private:
    ...
    uint8_t m_urllcUes{0};     //!< The number of URLLC UEs in the scheduler
    ...
    bool m_enableUrllcHarqReTx{true}; //!< Flag to enable or disable HARQ ReTx for URLLC packets (attribute)
    Ptr<LteEnbRrc> m_rrc{nullptr};
};



class NrMacSchedulerNs3ForDuplication: public NrMacSchedulerNs3{
    public:
    void DoSchedDlCqiInfoReq(const NrMacSchedSapProvider::SchedDlCqiInfoReqParameters& params);



    

    // In LteEnbRrc and UeManager classes, 
    // NrMacSchedulerNs3ForDuplication is defined as friend class.
    private:
