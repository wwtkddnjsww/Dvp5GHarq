    ...
    for (auto packetBurst : m_rxPacketBurstList)
    {
        ...
        for (auto packet : packetBurst->GetPackets())
        {
            ...
            if (enbRx)
            {
                ...
                if (burstLogged == false)
                {
                    // std::cout << "Logging burst reception at gNB, cellId " << traceParams.m_cellId
                    //           << " frame " << traceParams.m_frameNum
                    //           << " subframe "
                    //           << traceParams.m_subframeNum
                    //           << " slot " << traceParams.m_slotNum
                    //           << " TB size " << traceParams.m_tbSize
                    //           << " mcs " << static_cast<uint32_t>(traceParams.m_mcs)
                    //           << " rv " << +traceParams.m_rv
                    //           << " sinrAvg " << traceParams.m_sinr
                    //           << " sinrMin " << traceParams.m_sinrMin
                    //           << " tbler " << traceParams.m_tbler
                    //           << " corrupt " << traceParams.m_corrupt
                    //           << std::endl;
                    m_rxPacketTraceEnb(traceParams);
                    NS_LOG_INFO("Logging burst reception at gNB, cellId " << traceParams.m_cellId
                                                                          << " frame " << traceParams.m_frameNum
                                                                          << " subframe "
                                                                          << traceParams.m_subframeNum
                                                                          << " slot " << traceParams.m_slotNum);
