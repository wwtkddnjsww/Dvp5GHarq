...
void
NrInterference::SetNoiseAnalysis(bool noiseAnalysis){
    m_noiseAnalysis = noiseAnalysis;
}
...
NrInterference::EndRx()
{
    ...
    else
    {
        ...
        //std::cout<<"All signals: " << (*m_allSignals)[0] << ", rxSingal:" << (*m_rxSignal)[0] << " , noise:" << (*m_noise)[0]<<std::endl;
...
NrInterference::ConditionallyEvaluateChunk()
{
    ...
    if (m_receiving && (Now() > m_lastChangeTime))
    {
        ...
        //This part, we delete the interference for scheduling experiment. Sangwon
