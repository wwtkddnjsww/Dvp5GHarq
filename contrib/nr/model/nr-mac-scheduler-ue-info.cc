...
NrMacSchedulerUeInfo::NrMacSchedulerUeInfo(uint16_t rnti,
    ...
    else
    {
        ...
        case 1:
            if (m_dlMcs.size() == 1)
            {
                ...
                // std::cout<< "DL RBG: " << m_dlRBG << std::endl;
            ...
            else
            {
                ...
                NS_LOG_DEBUG("Max CQI " << static_cast<uint16_t>(*it)
                                        << " at index : " << maxCqiIndex);
                for (uint32_t stream = 0; stream < m_dlCqi.m_wbCqi.size(); stream++)
                {
                    if (stream == maxCqiIndex)
                    {
                        uint8_t mcs = m_dlMcs.at(stream);
                        NS_ASSERT_MSG(mcs != UINT8_MAX,
                                      "Invalid MCS " << +mcs << " for CQI "
                                                     << +m_dlCqi.m_wbCqi.at(stream)
                                                     << " for stream " << stream);
                        NS_LOG_DEBUG("Switching from 2 streams to 1. Using "
                                     << stream << " that has the maximum MCS : " << +mcs);
                        m_dlTbSize.at(stream) =
                            amc->CalculateTbSize(mcs, m_dlRBG * GetNumRbPerRbg());
                    }
                    else
                    {
            ...
            break;
        case 2:
            NS_ASSERT_MSG(m_dlMcs.at(0) != UINT8_MAX,
                          "Invalid MCS " << +m_dlMcs.at(0) << " for CQI " << +m_dlCqi.m_wbCqi.at(0)
                                         << " for stream 0");
            NS_ASSERT_MSG(m_dlMcs.at(1) != UINT8_MAX,
                          "Invalid MCS " << +m_dlMcs.at(1) << " for CQI " << +m_dlCqi.m_wbCqi.at(1)
                                         << " for stream 1");
...
NrMacSchedulerUeInfo::UpdateUlMetric(const Ptr<const NrAmc>& amc)
{
    ...
    {
        // std::cout << "UL RBG: " << m_ulRBG* GetNumRbPerRbg() << std::endl;
...
{
    // std::cout<< "call get num rb per rbg: " << m_getNumRbPerRbg() << std::endl; 
