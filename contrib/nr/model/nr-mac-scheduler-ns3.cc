    ...
    static TypeId tid =
        TypeId("ns3::NrMacSchedulerNs3")
            ...
            .AddAttribute("UrllcUes",
                          ...
                          UintegerValue(0),
                          MakeUintegerAccessor(&NrMacSchedulerNs3::SetUrllcUes,
                                               &NrMacSchedulerNs3::GetUrllcUes),
                          MakeUintegerChecker<uint8_t>())
            ...
            .AddAttribute("EnableHarqReTx",
                          ...
                          MakeBooleanChecker())
            .AddAttribute("EnableUrllcHarqReTx",
                          "If true, it would set the max HARQ ReTx to 3; otherwise it set it to 0",
                          BooleanValue(true),
                          MakeBooleanAccessor(&NrMacSchedulerNs3::EnableUrllcHarqReTx,
                                              &NrMacSchedulerNs3::IsUrllcHarqReTxEnable),
                          MakeBooleanChecker())
            .AddAttribute(
...
void
NrMacSchedulerNs3::SetUrllcUes(uint8_t v)
{
    NS_LOG_FUNCTION(this);
    m_urllcUes = v;
}
...
uint8_t
NrMacSchedulerNs3::GetUrllcUes() const
{
    NS_LOG_FUNCTION(this);
    return m_urllcUes;
}
...
void
NrMacSchedulerNs3::EnableUrllcHarqReTx(bool enableFlag)
{
    m_enableUrllcHarqReTx = enableFlag;
}

bool
NrMacSchedulerNs3::IsUrllcHarqReTxEnable() const
{
    return m_enableUrllcHarqReTx;
}
    ...
    const NrMacSchedSapProvider::SchedDlCqiInfoReqParameters& params)
{
    ...
    if (m_fixedMcsDl && m_urllcUes == 0)
    {
        // This case is not considering all URLLC packets but it can be used for the simulation when fixing MCS index
        return;
    }
    else if (m_fixedMcsDl && m_urllcUes != 0){
        // This case is used for URLLC packets and eMBB packets. Recommend to only activating this part when EDF scheduler activated.
        for (const auto& cqi : params.m_cqiList)
        {
            Ptr<UeManager> ueManager = m_rrc->m_ueMap.find(cqi.m_rnti)->second;
            // std::cout<< "IMSI" << ueManager->GetImsi() << "RNTI" << cqi.m_rnti << std::endl;
            if (ueManager->GetImsi() > m_urllcUes){
        ...
        // This case is used for basic simulation scenarios.
        for (const auto& cqi : params.m_cqiList)
        {
            NS_ASSERT(m_ueMap.find(cqi.m_rnti) != m_ueMap.end());
            const std::shared_ptr<NrMacSchedulerUeInfo>& ue = m_ueMap.find(cqi.m_rnti)->second;
            ...
            //This part is for Flexible repetition
            // if (m_rrc != nullptr){
            //     //Find Ue Manager
            //     Ptr<UeManager> ueManager = m_rrc->m_ueMap.find(cqi.m_rnti)->second;
                
            //     // Making all DRBs attached UE
            //     for (auto it = ueManager->m_drbMap.begin(); it != ueManager->m_drbMap.end(); ++it){
                    
            //         it->second->m_pdcp->SetDuplicationDelay(1);
            //         it->second->m_pdcp->SetNumberOfDuplications(2);
            //     }
    ...
    const std::string& direction) const
{
         ...
         /* nothing as increment */)
    {
        ...
        NS_ASSERT(*rvIt < 4);
        Ptr<UeManager> ueManager = m_rrc->m_ueMap.find(rnti)->second;
        uint8_t maxHarqReTx = m_enableHarqReTx == true ? 3 : 0;
        if (ueManager->GetImsi() <= m_urllcUes)
            maxHarqReTx = m_enableUrllcHarqReTx == true ? 3 : 0;
        // std::cout<<"UE ID: " << ueManager->GetImsi() << "UE HARQ: " << maxHarqReTx << std::endl;
                                   ...
                                   const std::string& mode) const
{
    ...
    for (const auto& ueInfo : m_ueMap)
    {
        ...
        if (mode == "UL"){
            totBuffer = 108; //Changed by Sangwon
        }
                                    ...
                                    SlotAllocInfo* slotAlloc) const
{
    ...
    for (const auto& beam : activeDl)
    {
        ...
        //Sangwon changes here, shuffle UE;
        //This is the implementation for diversity
        int numUE = beam.second.size();
        std::vector <int> UEArray;
        for (int nUE = 0; nUE < numUE; nUE++){
            UEArray.push_back(nUE);
        }
        bool randomshuffling = true;
        if (randomshuffling == true){
            std::random_device rd;
            //non-deterministic data generator
            std::mt19937 g(rd());

            std::shuffle(UEArray.begin(), UEArray.end(),g);
        }
        //The commented next line is original implementation
        //for (const auto& ue : beam.second){
        //from here
        for (const auto& ueNumber : UEArray)
        {

            const auto& ue = beam.second[ueNumber];
            //until here is newly implemented
                                    ...
                                    SlotAllocInfo* slotAlloc) const
{
    ...
    for (const auto& beam : activeUl)
    {
        ...
        for (const auto& ue : beam.second)
        {
            ...
            NS_LOG_INFO("Assigned process ID " << static_cast<uint32_t>(dci->m_harqProcess)
                                               << " to UE " << ue.first->m_rnti);
            // std::cout<<"Assigned process ID " << static_cast<uint32_t>(dci->m_harqProcess)
            //                                    << " to UE " << ue.first->m_rnti << std::endl;
            for (uint32_t stream = 0; stream < dci->m_tbSize.size(); stream++)
            {
                NS_LOG_DEBUG(" UE" << dci->m_rnti << " gets UL symbols "
                                   ...
                                   << static_cast<uint32_t>(id) << " rv "
                                   << static_cast<uint32_t>(dci->m_rv.at(stream)));
                // std::cout << " UE" << dci->m_rnti << " gets UL symbols "
                //                    << static_cast<uint32_t>(dci->m_symStart) << "-"
                //                    << static_cast<uint32_t>(dci->m_symStart + dci->m_numSym)
                //                    << " tbs " << dci->m_tbSize.at(stream) << " mcs "
                //                    << static_cast<uint32_t>(dci->m_mcs.at(stream)) << " harqId "
                //                    << static_cast<uint32_t>(id) << " rv "
                //                    << static_cast<uint32_t>(dci->m_rv.at(stream)) << std::endl;
            }
            ...
            for (const auto& byteDistribution : distributedBytes)
            {
                ...
                // std::cout<< "UL LCG " << static_cast<uint32_t>(byteDistribution.m_lcg)
                //                        << " assigned bytes " << byteDistribution.m_bytes
                //                        << " to LCID "
                //                        << static_cast<uint32_t>(byteDistribution.m_lcId) <<std::endl;
            }
            NS_ASSERT(assignedToLC);//Changed by Sangwon
    ...
    NS_ASSERT(spoint->m_rbg == 0);
    // std::cout << "Till Here it is fine" << std::endl;
    return usedSym;
}
...
void 
NrMacSchedulerNs3::SetRrc(Ptr<LteEnbRrc> rrc)
{
    m_rrc = rrc;
}

Ptr<LteEnbRrc>
NrMacSchedulerNs3::GetRrc()
{
    return m_rrc;
}
