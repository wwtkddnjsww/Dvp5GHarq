...
NrUeMac::ProcessUlDci(const Ptr<NrUlDciMessage>& dciMsg)
{
    ...
    {
        // std::cout << "Before sending NewData, bufSize " << GetTotalBufSize() << "Allocated TbSize: " << m_ulDci->m_tbSize.at(0) << std::endl;
        SendNewData();
        NS_LOG_INFO("After sending NewData, bufSize " << GetTotalBufSize());
        // std::cout << "After sending NewData, bufSize " << GetTotalBufSize() << std::endl;
        ...
        if (GetTotalBufSize() == 0)
        {
            ...
            if (m_ulDciTotalUsed == 0)
            {
                ...
                // DoTransmitPdu(txParams); //Changed by Sangwon to not send empty PDU, because it can cause some issues in the gNB side, and it is not mandatory to send an empty PDU when we have nothing to send. The HARQ timer at gNB will expire, and soon this allocation will be forgotten.
            }
        }
    }
}
...
NrUeMac::SendNewData()
{
    ...
    // std::cout << "Useful TBS after sending StatusPdu and reserving for SHORT_BSR: " << usefulTbs << std::endl;
    ...
    // std::cout << "ACTIVE LCS TX" << activeLcsTx << " ACTIVE LCS RETX " << activeLcsRetx << " Useful TBS after sending RETX data and reserving for SHORT_BSR: " << usefulTbs << std::endl;
